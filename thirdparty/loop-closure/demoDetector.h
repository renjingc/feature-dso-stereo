...
public:
   ...
   */
  demoDetector(const std::string &vocfile, int width, int height, float alpha);
...
demoDetector<TVocabulary, TDetector, TDescriptor>::demoDetector
  (const std::string &vocfile, int width, int height, float alpha)
  : m_vocfile(vocfile), m_width(width), m_height(height),
    params(height, width, 1, true, alpha), voc(vocfile), detector(voc, params)
{
