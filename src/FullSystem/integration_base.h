...
class IntegrationBase
{
  public:
    ...
    /**
     * @brief midPointIntegration
     * @param _dt    间隔时间
     * @param _acc_0 上一时刻加速度
     * @param _gyr_0 上一时刻角速度
     * @param _acc_1 当前时刻加速度
     * @param _gyr_1 当前时刻角速度
     * @param delta_p 上一时刻累积的位置
     * @param delta_q 上一时刻累积的角度
     * @param delta_v 上一时刻累积的速度
     * @param linearized_ba 上一时刻的加速度偏差
     * @param linearized_bg 上一时刻的角速度偏差
     * @param result_delta_p 计算后的位置
     * @param result_delta_q 计算后的角度
     * @param result_delta_v 计算后的速度
     * @param result_linearized_ba 计算后的加速度偏差
     * @param result_linearized_bg 计算后的角速度
     * @param update_jacobian 更新后的雅克比
     */
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        //角度累积,qk+1=qk⊗q{(ωk+nωk+ωk+1+nωk+12−ωbk)δt}
        result_delta_q = delta_q * Quaterniond(1, un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2, un_gyr(2) * _dt / 2);

        //(当前时刻的加速度-加速度偏差)乘以当前的角度累积，得到当前变换后的角速度
        Vector3d un_acc_1 = result_delta_q * (_acc_1 - linearized_ba);
        //两时刻的加速度均值
        Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1);

        //计算累积的位置,pk+1=pk+vkδt+12qk(ak+nak−abk)+qk+1(ak+1+nak+1−abk+1)2δt2
        result_delta_p = delta_p + delta_v * _dt + 0.5 * un_acc * _dt * _dt;
        //计算累积的速度,vk+1=vk+qk(ak+nak−abk)+qk+1(ak+1+nak+1−abk+1)2δt
        ...
        if(update_jacobian)
        {
            //当前角速度=角速度均值-角速度偏差,ωbk+1=ωbk+nwbδt
            Vector3d w_x = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg;
            //上一时刻加速度,abk+1=abk+nabδt
            ...
            //Jt+δt=(I+Ftδt)Jt
            jacobian = F * jacobian;
            //更新IMU协方差矩阵 Pbkt+δt=(I+Ftδt)Pbkt(I+Ftδt)T+GtδtQ(Gtδt)T
    ...
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        //计算雅克比矩阵 jacobian = F*jacobian，计算协方差矩阵
