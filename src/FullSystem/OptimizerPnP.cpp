          ...
          SE3& outT)
{
    ...
    else
    {
        ...
        cv::Mat R, t;
        cv::solvePnPRansac(p3d, p2d, K, cv::Mat(), R, t, false, 100, 4.0, 0.99, inliers,CV_EPNP);

        // int cntInliers = 0;

        for (size_t k = 0; k < inliers.rows; k++)
        {
            int index = inliers.at<int>(k, 0);
            cntInliers++;
        }
        if( cntInliers < 5)
            return false;
...
int PoseOptimization(const cv::Mat& mTcw, const std::vector<cv::KeyPoint>& mvKeys,
                                           const std::vector<float>& mvuRight,const std::vector<cv::KeyPoint>& mvKeysUn,
