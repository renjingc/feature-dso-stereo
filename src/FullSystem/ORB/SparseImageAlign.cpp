...
size_t SparseImgAlign::run ( FrameHessian* ref_frame, FrameHessian* cur_frame )
{
    ...
    ref_patch_cache_ = cv::Mat ( ref_frame_->_features.size(), patch_area_, CV_32F );
...
void SparseImgAlign::precomputeReferencePatches()
{
    ...
    const double focal_length = ref_frame_->_camera->focal();
    size_t feature_counter = 0;
    std::vector<bool>::iterator visiblity_it = visible_fts_.begin();

    for ( auto it = ref_frame_->_features.begin(), ite = ref_frame_->_features.end();
            it != ite; ++it, ++feature_counter, ++visiblity_it )
    {
        // check if reference with patch size is within image
        const float u_ref = ( *it )->_pixel[0] * scale;
        const float v_ref = ( *it )->_pixel[1] * scale;
        const int u_ref_i = floorf ( u_ref );
        const int v_ref_i = floorf ( v_ref );
        if ( ( *it )->_mappoint == nullptr || u_ref_i - border < 0 || v_ref_i - border < 0 || u_ref_i + border >= ref_img.cols || v_ref_i + border >= ref_img.rows )
            continue;
        *visiblity_it = true;
        ...
        frame_jac = cvutils::JacobXYZ2Cam( xyz_ref );
    ...
    bool compute_weight_scale )
{
    ...
    std::vector<bool>::iterator visiblity_it = visible_fts_.begin();
    for ( auto it = ref_frame_->_features.begin(); it != ref_frame_->_features.end();
